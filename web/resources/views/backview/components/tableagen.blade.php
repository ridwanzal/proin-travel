<div class="row mb-5">
    <div class="card">
        <div class="card-body">
            <h6>Aktifkan Form / CTA di Pendaftaran Agen</h6>
                ...
                <tbody>
                    @foreach($statusForm as $item)
                        <tr>
                            ...
                            <td>
                                @if($item->type == 1)
                                    <form method="POST" action="{{ url('/daftaragen/update') }}">
                                        @csrf
  ...
  <tbody>
        ...
        @foreach ($daftaragen as $item)
            <tr>
                ...
                <td>{{$item->nama}}</td>
                <td id="dataCopy">{{$item->whatsapp}}</td>
                <td>{{$item->domisili}}</td>
                <td>
                    <button id="{{$i}}" class="btn btn-sm btn-success" onclick="copyTeks()">Salin nomor WA</button>
                </td>
            </tr>
        @endforeach
  </tbody>
